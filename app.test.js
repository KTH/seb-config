...
const courseID = 1234
let testConfigCourseID= sebCg.generateSEBConfig(courseID, 1)
let testConfigUniversal= sebCg.generateSEBConfig(courseID, 0)
...
test('4. CourseID Regex Pass', () => {
  var testRegArr = (testConfigCourseID.elements[1].elements[0].elements[245].elements[0].text).split(';')
  var testPatt = new RegExp(testRegArr[1])
  var result = testPatt.test(`canvas.kth.se/courses/${courseID}`)
  expect(result).toBeTruthy();
});
test('5. Universal Regex Pass', () => {
  var testRegArr = (testConfigUniversal.elements[1].elements[0].elements[245].elements[0].text).split(';')
  var testPatt = new RegExp(testRegArr[1])
  var result = testPatt.test(`canvas.kth.se/courses/4567`)
  expect(result).toBeTruthy();
});
